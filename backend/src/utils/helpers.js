...
/**
 * Automatically delete an old file from Supabase Storage 'uploads' bucket
 * (or local disk fallback) when it is replaced or deleted.
 */
async function deleteStorageFile(url) {
  ...
  try {
    ...
    // 1. Supabase Storage bucket 'uploads'
    if (url.includes('/storage/v1/object/public/uploads/')) {
      const parts = url.split('/storage/v1/object/public/uploads/');
      const filePath = parts[1]; // e.g. "uploads/1723456789-123.png" or "1723456789-123.png"
      if (filePath) {
    ...
    console.warn('[Storage Cleanup] Notice:', err?.message || err);
  }
}

module.exports = { ok, generateOrderCode, generatePaymentCode, generateKeyCode, maskKey, sanitizeLimit, sanitizePage, sanitizeSearch, slugify, deleteStorageFile };

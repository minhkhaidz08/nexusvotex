...
const listDeposits = asyncHandler(async (req, res) => {
  ...
  if (paymentIds.length > 0) {
    ...
    if (txs) {
      for (const t of txs) {
        txMap[t.ref_id] = Number(t.amount);
      }
    }
  }

  const result = (payments || []).map((p) => {
    let receivedAmount = txMap[p.id];
    if (receivedAmount === undefined || receivedAmount === null) {
      if (p.detail && p.detail.credit_amount !== undefined) {
        receivedAmount = Number(p.detail.credit_amount);
      } else {
    ...
    return {
      ...p,
      received_amount: Math.abs(receivedAmount),
    };
  });

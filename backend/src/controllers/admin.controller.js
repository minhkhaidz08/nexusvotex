...
const createProduct = asyncHandler(async (req, res) => {
  ...
  // Every product must have at least one variant (loại key). The product-level
  // price stays as the reference; each variant carries its own price.
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  if (patch.seller_price !== undefined) {
    ...
    if (patch.seller_price !== null && !(patch.seller_price >= 0)) throw new AppError('seller_price must be a non-negative number');
  }
  ...
  if (req.body.variants !== undefined) {
    ...
    if (removedIds.length > 0) {
      await admin.from('product_variants').delete().in('id', removedIds);
    }
    for (const v of variants) {
      ...
      else await admin.from('product_variants').insert({ product_id: id, ...row });
    }
  }

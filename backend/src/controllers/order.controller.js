...
async function computeCart(items, role) {
  ...
  const { data: products, error } = await admin
    .from('products')
    .select('id, name, price, seller_price, type, image_url, is_active')
    .eq('is_active', true)
    .in('id', productIds);
  ...
  for (const item of items) {
    ...
    const unit = buyerPrice(role, product);
    subtotal += unit * qty;
    lines.push({ product: { ...product, price: unit }, qty });
  }
 ...
 */
async function claimKeysBatch({ userId, product, qty, orderCode, perKeyTotals, perKeyDiscounts }) {
...
const createCustomOrder = asyncHandler(async (req, res) => {
  ...
  const { data: product, error: pErr } = await admin
    .from('products')
    .select('id, name, price, seller_price, type, image_url')
    .eq('id', product_id)
    .eq('is_active', true)
    .maybeSingle();
  ...
  const amount = buyerPrice(req.user?.role, product) * numQty;
  await debitForPurchase({
    userId: req.userId,
    amount,
